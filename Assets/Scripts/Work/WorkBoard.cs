...
{
    /// Workplace board: holds tickets related to a workplace.
    /// Same ticket lifecycle as HomeBoard.
    public sealed class WorkBoard : MonoBehaviour
    {
        ...
        [Header("Ownership")]
        [Tooltip("Workplace identifier. Citizens with matching CitizenIdentity.workplaceId are eligible for WorkplaceOnly tickets.")]
        [Min(0)] public int workplaceId = 0;

        private int _nextTicketId = 1;

        [Header("Stale reservation handling")]
        [Min(1)] public int staleTimeoutTicks = 200;
        ...
        public TicketId AddTicket(ref BoardTicket ticket)
        {
            ...
            for (int i = 0; i < tickets.Count; i++)
            {
                ...
                {
                    Debug.LogWarning($"WorkBoard: duplicate TicketId '{ticket.id}'. Ticket not added.");
                    return ticket.id;
                }
            }
        ...
        {
            if (!TryGetTicketById(id, out var t, out int idx)) return false;
            if (t.state != TicketState.Open) return false;
        ...
        {
            if (!TryGetTicketById(id, out var t, out int idx)) return false;
            if (t.reservedByCitizenId != citizenId) return false;
            if (t.state != TicketState.Reserved) return false;
        ...
        public bool TryComplete(TicketId id, int citizenId, string notes = null)
        {
            if (!TryGetTicketById(id, out var t, out int idx)) return false;
            if (t.reservedByCitizenId != citizenId) return false;
            if (t.state != TicketState.InProgress) return false;

            t.state = TicketState.Done;
            if (!string.IsNullOrEmpty(notes)) t.notes = notes;
            tickets[idx] = t;
            return true;
        }
        ...
        public bool TouchProgress(TicketId id, int citizenId, int nowTick, string notes = null)
        {
            if (!TryGetTicketById(id, out var t, out int idx)) return false;
            if (t.reservedByCitizenId != citizenId) return false;
            if (t.state != TicketState.Reserved && t.state != TicketState.InProgress) return false;
        ...
        public void ReleaseStaleReservations(int nowTick)
        {
            ...
            {
                var t = tickets[i];
                if (t.state != TicketState.Reserved && t.state != TicketState.InProgress) continue;
                if (t.reservedByCitizenId == BoardTicket.UnreservedCitizenId) continue;
